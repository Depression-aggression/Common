...
public class EditorButton : Editor
{
    ...
    private void DrawButtonForMethod(IEnumerable<object> invokationTargets, MethodInfo methodInfo, EditorButtonState state)
    {
        EditorGUILayout.BeginHorizontal();
        var foldoutRect = EditorGUILayout.GetControlRect(GUILayout.Width(10.0f));
        state.isOpened = EditorGUI.Foldout(foldoutRect, state.isOpened, "");
        var clicked = GUILayout.Button(MethodDisplayName(methodInfo), GUILayout.ExpandWidth(true));
    ...
    private ParameterDrawer GetParameterDrawer(ParameterInfo parameter)
    {
        ...
        if (_typeDrawer.TryGetValue(parameterType, out var drawer))
            return drawer;
